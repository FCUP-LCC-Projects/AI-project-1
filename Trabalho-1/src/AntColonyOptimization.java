...
public class AntColonyOptimization{
        ...
        private double getTotalPheromone(Solution [] sols, int i, int j){
            ...
            Neighbours t = new Neighbours(mem);
            for(int k=0; k<sols.length; k++){
                Solution curr = sols[k];
                int conflicts = t.conflicts(curr, true);
        ...
        public void findBestPath(int maxItrs, int ants){
            ...
            int bestPerimeter = Integer.MAX_VALUE/2;
            int bestIndex=0;

            Neighbours t = new Neighbours(mem);
            
            for(int i=0; i<ants; i++){
                int conflicts = t.conflicts(solutions[i],true);
                int perimeter = solutions[i].getPerimiter();
            ...
            solutions[bestIndex].printSolution(mem);
